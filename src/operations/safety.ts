...
function isInGracePeriod(fullyQualifiedName: string) {
	...
	return Promise.all([network.getNameInfo(fullyQualifiedName), network.getBlockHeight(), network.getGracePeriod()])
...
function addressCanReceiveName(address: string) {
	return config.network.getNamesOwned(address).then(names => names.length < 25);
}
